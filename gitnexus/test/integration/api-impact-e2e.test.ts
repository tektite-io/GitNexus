...
withTestLbugDB('api-impact-e2e', (handle) => {
  ...
  describe('error shape separation', () => {
    ...
    it('consumer accessing error key does not trigger mismatch', async () => {
      ...
      const grantsListMismatches = result.mismatches?.filter(
  ...
  describe('tool consistency across route_map, shape_check, api_impact', () => {
    ...
    it('all tools return the same handler file', async () => {
      ...
      const rmHandler = routeMapResult.routes.find((r: any) => r.route === '/api/grants')?.handler;
      const scHandler = shapeCheckResult.routes.find((r: any) => r.route === '/api/grants')?.handler;
